...
# and error), the number of observations, and 2x the
...
def makeMBpgram(pgrams,lcs,nobs,nh2):
	...
	for i,p in enumerate(pgrams):
		#for different number of observations per band, need to construct d2 on a per-band basis
		d2 = float(nobs[i]-nh2-1)
		d2_tot += d2
		data = np.array(lcs[i])
		rw = 1./data[:,2]
		m = data[:,1]
	...
	MBpg = d2_tot*c2/(d1*(vrf_tot-c2))
	return MBpg

# Fix seed for demonstrative purposes
np.random.seed(5)
bands = ['u','g','r','i','z']
#nh2 is 2x the number of harmonics to be used
nh2 = 2
n = [10,13,12,17,11]
lcs = []
